...
def update_mouse_position(hough_circles, eye_x_pos, eye_y_pos, roi_color2):
    try:
        for circle in hough_circles[0, :]:
            ...
            cv2.circle(
                img=roi_color2, center=circle_center, radius=2, color=WHITE, thickness=3
            )
            ...
            pyautogui.mouse.move(x_pos, y_pos)
...
x = sns.kdeplot(
    eye_x_positions, eye_y_positions, shade=True, cmap="rocket_r", shade_lowest=False
)
plt.tight_layout(pad=1)
plt.axis("off")
